...
describe('countdownParts', () => {
	it('splits a remaining duration into days, hours, minutes', () => {
		const now = 1_700_000_000_000;
		expect(countdownParts(now + 3 * DAY + 5 * HOUR + 42 * MINUTE, now)).toEqual({
			days: 3,
			hours: 5,
			minutes: 42,
			expired: false,
	...
	it('floors partial minutes (59s left displays as 0m, not expired)', () => {
		const now = 1_700_000_000_000;
		expect(countdownParts(now + 59_000, now)).toEqual({
			days: 0,
			hours: 0,
			minutes: 0,
			expired: false,
	...
	it('rolls hours over at 24 and minutes at 60', () => {
		const now = 1_700_000_000_000;
		...
		expect(countdownParts(now + 25 * HOUR + 61 * MINUTE, now)).toEqual({
			days: 1,
			hours: 2,
			minutes: 1,
			expired: false,
	...
	it('clamps to zeros and reports expired once the end has passed', () => {
		const now = 1_700_000_000_000;
		expect(countdownParts(now, now)).toEqual({ days: 0, hours: 0, minutes: 0, expired: true });
