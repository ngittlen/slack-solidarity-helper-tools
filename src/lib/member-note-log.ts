...
export interface NoteLogInput {
	 ...
	 *  it did. */
	dmBody: string | null;
	/** Permalink to the Slack message the note is about, or null when the admin
	 *  attached none. Always shown when present so an admin reading the channel
	 *  can jump straight to what the note is describing. */
 ...
 * plus ` — <link|regarding this message>` whenever a permalink was attached.
 */
export function renderMemberNoteLog(input: NoteLogInput): string {
	...
	// Trailing rather than inline: the sentence already names two users, and a
	// link parked at the end stays clickable without breaking up the reading.
	const url = input.messageLink?.trim() ?? '';
	const link = url ? ` — <${url}|regarding this message>` : '';
	return `${label} “${body}” added to user <@${input.targetSlackUserId}>${sent} by <@${input.authorSlackUserId}>${link}`;
}
