<script lang="ts">
	...
	let totalPending = $derived(
		data
			? data.pending.filter((e) => (statusState[e.id] ?? e.status) !== 'verified_in_slack').length
			: 0,
	);
	...
	let hasUncontactedEmails = $derived(
		data
			? data.pending.some((e) => (statusState[e.id] ?? e.status) === 'uncontacted' && e.email)
			: false,
	);
	...
	async function copyUncontactedEmails() {
		...
		copyLabel = 'Copied!';
		setTimeout(() => {
			copyLabel = 'Copy uncontacted emails';
		}, 2000);
	}
...
<main>
	...
	{:else if data}
		{#if !sseConnected}
			<p class="sse-error">
				Live updates disconnected — changes from other users won't appear until you reload.
			</p>
		{/if}
		...
		<div class="toolbar">
			...
			<button
				onclick={copyUncontactedEmails}
				disabled={!hasUncontactedEmails}
				title={!hasUncontactedEmails ? 'There are no uncontacted people' : undefined}
				>{copyLabel}</button
			>
		</div>
				...
				<table>
					...
					<tbody>
						{#each sortedEntries as entry (entry.id)}
							...
							<tr
								class:helped={currentStatus === 'verified_in_slack'}
								class:contacted={currentStatus === 'contacted'}
							>
								<td class="col-helped">
									<select
										value={currentStatus}
								...
								<td class="col-comment">
									<textarea
										...
										aria-label="Comment for {entry.name ??
											entry.email ??
											entry.phone ??
											'volunteer'}"
										bind:value={comments[entry.id]}
										oninput={() => onCommentInput(entry.id)}
										class:unsaved={saveStatuses[entry.id] === 'pending' ||
											saveStatuses[entry.id] === 'saving'}
										class:saved={saveStatuses[entry.id] === 'saved'}></textarea>
